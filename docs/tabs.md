...
After upgrade, Manner applies `tablist`, `tab`, and `tabpanel` semantics; selected state; `aria-controls`; `aria-labelledby`; roving `tabindex`; and `hidden` to inactive panels. It does not add CSS or create content.
